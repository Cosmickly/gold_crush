...
namespace Tiles
{
    public class GroundTile : MonoBehaviour
    {
        ...
        private void Start()
        {
            float newY = Random.Range(0.01f, 0.04f);
        ...
        private void FixedUpdate()
        {
            ...
            float crackRatio = 1 - _crackTimer / _crackTime;
            _meshRenderer.material.color =
                Color.Lerp(_initialColor, _initialColor * new Color(0.2f, 0.2f, 0.2f), crackRatio);

            float newY = Mathf.Sin(Time.fixedTime * Mathf.PI * _frequency * crackRatio) * (_amplitude * crackRatio);
            _meshObject.transform.localPosition = new Vector3(0, newY, 0);
        }
